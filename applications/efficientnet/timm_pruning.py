...
def train_one_epoch(
    ...
    task_loss_m = utils.AverageMeter()
    flop_loss_m = utils.AverageMeter()
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if batch_idx % base_model.scheduler.structure_log_interval == 0:
            ...
            print_log(
                f"target: {base_model.scheduler.current_target(*base_model.runtime_info)}"
            )
        ...
        def _forward():
            with amp_autocast():
                ...
                task_loss = loss_fn(logits, target)
                loss = task_loss + flop_loss
                ##############################################################
            if accum_steps > 1:
                loss /= accum_steps
            return loss, task_loss, flop_loss
        ...
        def _backward(_loss):
            ...
            if loss_scaler is not None:
                ...
                base_model.scheduler.norm_grad()
        ...
        if has_no_sync and not need_update:
            ...
            loss, task_loss, flop_loss = _forward()
            _backward(loss)

        if not args.distributed:
            losses_m.update(loss.item() * accum_steps, input.size(0))
            task_loss_m.update(task_loss.item(), input.size(0))
            flop_loss_m.update(flop_loss.item(), input.size(0))
        ...
        if update_idx % args.log_interval == 0:
            ...
            if utils.is_primary(args):
                _logger.info(
                    ...
                    f"task: {task_loss_m.val:#.3g} ({task_loss_m.avg:#.3g})  "
                    f"flop: {flop_loss_m.val:#.3g} ({flop_loss_m.avg:#.3g})  "
                    f"Time: {update_time_m.val:.3f}s, {update_sample_count / update_time_m.val:>7.2f}/s  "
                    f"({update_time_m.avg:.3f}s, {update_sample_count / update_time_m.avg:>7.2f}/s)  "
                    f"LR: {lrl}  "
                    f"flop_loss: {base_model.mutator.last_soft_flop/1e6}   "
                    f"Data: {data_time_m.val:.3f} ({data_time_m.avg:.3f})   "
                    f"e_norm: {base_model.mutator.e_norm:.2f}  "
                    f"e_flop_norm: {base_model.mutator.e_flop_norm:.2f}  "
                    f"target: {base_model.scheduler.current_target(*base_model.runtime_info):.2f}"
                )
